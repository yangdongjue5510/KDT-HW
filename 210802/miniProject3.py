...
SMTP_PORT = 465
SMTP_USER = 'test73015510@gmail.com'
SMTP_PASSWORD = 'yang9745510'
...
wb = Workbook()
ws = wb.create_sheet('receiverList')

receiverCount = 0
while True:
    receiverName = input('이름(그만 입력할 경우 -1 입력): ')
    if receiverName == '-1': break
    receiverAddr = input('이메일: ')
    receiverCount += 1
    print(receiverCount, receiverName, receiverAddr)
    ws.append([receiverCount, receiverName, receiverAddr])
...
loadedWb = load_workbook('email_list.xlsx')
data = loadedWb['receiverList']
rows= data['1:'+str(receiverCount)]
...
for row in rows:
    receiverArr=[]
    for cell in row:
        receiverArr.append(cell)
    send_mail(receiverArr[1].value, receiverArr[2].value, 'Crawling news', contents,excelName)